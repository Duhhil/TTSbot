...
- `.env` — token do bot (não compartilha esse arquivo com ninguém)
- `configs_usuarios.json` — configuração de cada usuário (criado automaticamente)
- `bot_language.json` — idioma atual dos comandos do bot, en ou pt (criado automaticamente)
- `audios/` — áudios temporários gerados, apagados depois de tocar
- `logs/` — log do bot, com rotação automática (máximo 5MB por arquivo, guarda até 5 antigos)